<div class="navbar-custom topnav-navbar topnav-navbar-dark">
    <div class="container-fluid">
        @php
            ...
            $reqMenunggu = RequestSuplyBarang::where('status', 'menunggu')
                ->latest()
                ->get();
        @endphp
                            ...
                            @foreach ($reqMenunggu as $i)
                                <a href="{{ url('/req-barang') }}" class="dropdown-item notify-item">
                                    <div class="notify-icon">
                                        <img src="{{ asset('img/profile/' . $i->user->foto) }}"
                                            class="img-fluid rounded-circle" alt="" />
                                    </div>
                                    <p class="notify-details">{{ $i->user->name }}</p>
                                    <p class="text-muted mb-0 user-msg">
                                        <small><strong>{{ $i->status }}</strong> ~ {{ $i->barangkeluar->count() }}
                                            Items</small>
                                    </p>
                                </a>
                            @endforeach
            ...
            <li class="dropdown notification-list">
                <a class="nav-link dropdown-toggle nav-user arrow-none me-0" data-bs-toggle="dropdown"
                    ...
                    <span class="account-user-avatar">
                        <img src="{{ asset('img/profile/' . Auth::user()->foto) }}" alt="user-image"
