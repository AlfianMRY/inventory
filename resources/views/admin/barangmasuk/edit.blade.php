    ...
    <div class="row">
        <div class="col-md-6 mb-3">
            ...
            <select class="form-select select-option" name="barang" >
                ...
                @foreach ($barang as $b)
                    <option value="{{ $b->id }}" {{ $data->barang_id == $b->id ? 'selected' : '' }}>{{ $b->nama }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-6 mb-3">
            ...
            <select class="form-select select-option" name="supplier" >
                ...
                @foreach ($supplier as $s)
                    <option value="{{ $s->id }}" {{ $data->supplier_id == $s->id ? 'selected' : '' }}>{{ $s->nama }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-6 mb-3">
            <label for="example-date" class="form-label">Date</label>
            <input class="form-control" id="example-date" value="{{ $data->tanggal_masuk }}" type="date" name="tgl_masuk">
        </div>
        <div class=" col-md-6 mb-3">
            <label class="form-label">Jumlah Barang Masuk</label>
            <input data-toggle="touchspin" value="{{ $data->quantity }}" type="text" value="1" name="quantity" min="1" data-bts-button-down-class="btn btn-danger" data-bts-button-up-class="btn btn-success">
        </div>
    </div>
