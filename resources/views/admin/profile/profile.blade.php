    ...
    <table id="basic-datatable" class="table table-striped dt-responsive nowrap w-100">
        ...
        <tbody>
            @php
                $no = 1;
            @endphp
            @foreach ($user->requestsuplybarang as $d)
                @php
                ...
                <tr>
                    ...
                    <td colspan="2">
                        <p>
                            <button class="btn btn-sm btn-primary w-full" type="button" data-bs-toggle="collapse"
                                data-bs-target="#collapseWidthExample-{{ $loop->index }}" aria-expanded="false">
                                List Barang :
                            </button>
                        </p>
                        <div class="collapse mb-0" id="collapseWidthExample-{{ $loop->index }}">
                            <div class="card card-body p-1">
                                @foreach ($d->barangkeluar as $i)
                                    <div class="row justify-between">
                                        <div class="col">{{ $i->barang->nama }}</div>
                                        <div class="col ms-auto">{{ $i->stock }}</div>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    </td>
                    {{-- <td>{{ $d->barangkeluar->nama }}</td>
                <td>{{ $d->quantity }}</td> --}}
